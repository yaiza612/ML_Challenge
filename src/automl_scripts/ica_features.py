...
def create_search_space_rf():
    ...
    fun = Categorical("fun", ("logcosh", "exp", "cube"))
    ica_whiten = Categorical("ica_whiten", ("arbitrary-variance", "unit-variance"))
    ica_components = Integer("ica_components", bounds=(3, 100), log=True)
    ...
    cs.add([feature_fit, fun, ica_whiten, ica_components, criterion, max_depth, min_samples_leaf, max_features,
...
def target_function(config, fidelity):
    ...
    try:
        assert config["max_features"] <= config["ica_components"]
