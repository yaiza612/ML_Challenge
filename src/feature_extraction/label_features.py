...
def compute_label_features(dummy, filtered_segment_data, labels=None):
    """
    ...
    features_dict = {'label': []}
    for ch in range(5):
        features_dict['label'].append(labels[ch])
    #if labels is not None:
    #    for ch in range(5):
    #        features_dict[f'label'] = [labels[ch]] * 5
...
if __name__ == "__main__":
    ...
    for i in range(6):
        filename = f'{path_for_features}eeg_label_features_data_{i}.csv'
        if not os.path.isfile(filename):
            ...
            for idx in tqdm(range(data.shape[1]), desc=f"Processing data of index {i} for labels"):
                ...
                time_domain_features_df = compute_label_features(segment_data, labels=segment_labels)
